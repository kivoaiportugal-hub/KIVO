...
const GLOVO_CONFIG = {
  id: "glovo",
  name: "Glovo",
  authUrl: "https://partners.glovoapp.com/oauth/authorize",
  tokenUrl: "https://partners.glovoapp.com/oauth/token",
  apiUrl: "https://api.glovoapp.com/v1",
  scopes: ["orders", "store", "reviews", "financials"],
};

export class GlovoConnector implements PlatformConnector {
  private config = GLOVO_CONFIG;

  getAuthUrl(restaurantId: string, redirectUri: string): string {
    const params = new URLSearchParams({
      response_type: "code",
      client_id: process.env.GLOVO_CLIENT_ID!,
  ...
  async handleCallback(code: string, restaurantId: string): Promise<PlatformToken> {
    const response = await fetch(this.config.tokenUrl, {
      ...
      body: new URLSearchParams({
        ...
        client_id: process.env.GLOVO_CLIENT_ID!,
        client_secret: process.env.GLOVO_CLIENT_SECRET!,
        code,
        redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/platforms/glovo/callback`,
      }),
    });
